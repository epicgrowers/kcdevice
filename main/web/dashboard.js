...
async function loadSensors(){const list=document.getElementById('sensor-list');try{if(list)list.innerHTML='<p class="text-gray-600 dark:text-gray-400">Loading sensors...</p>';const res=await fetch('/api/sensors',{signal:AbortSignal.timeout(30000)});if(!res.ok)throw new Error('Failed to load sensors');const d=await res.json();updateSensorDetailCache(Array.isArray(d.sensors)?d.sensors:[]);if(!list)return;if(!d.count){list.innerHTML='<p class="text-gray-600 dark:text-gray-400">No sensors detected</p>';return;}list.innerHTML=d.sensors.map(renderSensorCard).join('');}catch(e){console.error('Failed to load sensors:',e);if(list){const isTimeout=e.name==='TimeoutError'||e.message.includes('timed out');if(isTimeout){list.innerHTML=`<p class='text-orange-500'>⚠️ Sensor loading timed out. <button onclick='loadSensors()' class='underline'>Retry</button></p>`;}else{list.innerHTML=`<p class='text-red-500'>Failed to load sensors: ${e.message}. <button onclick='loadSensors()' class='underline'>Retry</button></p>`;}}}}
