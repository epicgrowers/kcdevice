...
function renderSensorCard(sensor){const caps=Array.isArray(sensor.capabilities)?sensor.capabilities:[];const addrHex=formatAddress(sensor.address);const type=(sensor.type||'').trim();const typeKey=type.toUpperCase();const isPh=typeKey==='PH';const isOrp=typeKey==='ORP';const isRtd=typeKey==='RTD';const isEc=typeKey==='EC';const isDo=typeKey==='DO';const isHum=typeKey==='HUM';const supportsCalibration=caps.includes('calibration')||isPh||isOrp||isRtd||isEc||isDo;const supportsTempComp=(caps.includes('temp_comp')&&isPh)||isPh;let html=`<div class='bg-gray-50 dark:bg-gray-700 p-4 rounded-lg mb-4 border-2 border-gray-200 dark:border-gray-600' id='sensor-card-${sensor.address}' data-sensor-address='${sensor.address}' data-sensor-type='${type}'>`;html+=`<h3 class='text-lg font-bold text-green-600 dark:text-green-400 mb-1'>${type||'Sensor'} @ 0x${addrHex}</h3>`;if(sensor.firmware)html+=`<p class='text-sm text-gray-600 dark:text-gray-400 mb-4'>Firmware: ${sensor.firmware}</p>`;if(type!=='MAX17048'){html+=`<div class='border-t border-gray-300 dark:border-gray-600 pt-4 mt-2'><h4 class='text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3'>Basic Configuration</h4>`;html+=`<div class='mb-4'><label class='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'>Sensor Name</label><input class='w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white' id='name-${sensor.address}' value='${sensor.name||''}' placeholder='Pool_pH' maxlength='16' pattern='[A-Za-z0-9_]+'><p class='text-xs text-gray-500 dark:text-gray-400 mt-1'>1-16 characters: letters, numbers, underscore only</p></div>`;html+=`<div class='mb-4'><label class='flex items-center text-gray-700 dark:text-gray-300'><input type='checkbox' class='mr-2' id='led-${sensor.address}' ${sensor.led?'checked':''}> LED On</label></div>`;if(isRtd){html+=`<div class='mb-4'><label class='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'>Temperature Scale</label><select class='w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white' id='scale-${sensor.address}'><option ${sensor.scale==='C'?'selected':''}>C</option><option ${sensor.scale==='F'?'selected':''}>F</option><option ${sensor.scale==='K'?'selected':''}>K</option></select></div>`;}if(isEc){html+=`<div class='mb-4'><label class='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'>Probe K Value</label><input class='w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white' type='number' step='0.1' id='probe-${sensor.address}' value='${sensor.probe_type??1.0}'></div>`;html+=`<div class='mb-4'><label class='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'>Output Parameters</label><div class='flex flex-wrap gap-3 mb-2'><label class='flex items-center'><input type='checkbox' id='param-ec-${sensor.address}' ${sensor.param_ec===true?'checked':''} class='mr-2'> EC</label><label class='flex items-center'><input type='checkbox' id='param-tds-${sensor.address}' ${sensor.param_tds===true?'checked':''} class='mr-2'> TDS</label><label class='flex items-center'><input type='checkbox' id='param-s-${sensor.address}' ${sensor.param_s===true?'checked':''} class='mr-2'> Salinity</label><label class='flex items-center'><input type='checkbox' id='param-sg-${sensor.address}' ${sensor.param_sg===true?'checked':''} class='mr-2'> Sp. Gravity</label></div><button class='w-full px-3 py-1.5 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded' onclick='updateEcOutputParams(${sensor.address})'>Update Output Parameters</button></div>`;}if(isHum){html+=`<div class='mb-4'><label class='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'>Output Parameters</label><div class='flex flex-wrap gap-3 mb-2'><label class='flex items-center'><input type='checkbox' id='param-hum-${sensor.address}' ${sensor.param_hum?'checked':''} class='mr-2'> Humidity</label><label class='flex items-center'><input type='checkbox' id='param-t-${sensor.address}' ${sensor.param_t?'checked':''} class='mr-2'> Temperature</label><label class='flex items-center'><input type='checkbox' id='param-dew-${sensor.address}' ${sensor.param_dew?'checked':''} class='mr-2'> Dew Point</label></div><button class='w-full px-3 py-1.5 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded' onclick='updateHumOutputParams(${sensor.address})'>Update Output Parameters</button></div>`;}html+=`<button class='w-full bg-green-600 dark:bg-green-400 hover:bg-green-700 dark:hover:bg-green-500 text-white px-4 py-2 rounded-md font-semibold' onclick='saveSensorConfig(${sensor.address})'><i class='fas fa-save'></i> Save ${type||'Sensor'} Settings</button></div>`;}if(supportsCalibration){html+=`<div class='border-t border-gray-300 dark:border-gray-600 pt-4 mt-4'><h4 class='text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3'>Calibration</h4><button class='w-full bg-gradient-to-r from-green-600 to-green-500 hover:from-green-700 hover:to-green-600 text-white px-6 py-3 rounded-lg font-semibold shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2' onclick='openCalibrationWizard({address:${sensor.address},type:"${type}",name:"${sensor.name||''}"});'><i class='fas fa-magic'></i> Start Calibration Wizard</button></div>`;html+=`<div class='border-t border-gray-300 dark:border-gray-600 pt-4 mt-4'><h4 class='text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3'>Diagnostics & Advanced</h4><div class='flex flex-wrap gap-2 mb-3'><button class='px-3 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded' onclick='findSensor(${sensor.address})'><i class='fas fa-search-location'></i> Find</button><button class='px-3 py-2 text-sm bg-purple-600 hover:bg-purple-700 text-white rounded' onclick='getDeviceStatus(${sensor.address})'><i class='fas fa-info-circle'></i> Status</button><button class='px-3 py-2 text-sm bg-green-600 hover:bg-green-700 text-white rounded' onclick='exportCalibration(${sensor.address})'><i class='fas fa-download'></i> Export</button><button class='px-3 py-2 text-sm bg-green-600 hover:bg-green-700 text-white rounded' onclick='importCalibration(${sensor.address})'><i class='fas fa-upload'></i> Import</button><button class='px-3 py-2 text-sm bg-orange-600 hover:bg-orange-700 text-white rounded' onclick='memoryClear(${sensor.address})'><i class='fas fa-eraser'></i> Clear Cal</button>`;if(isPh){html+=`<button class='px-3 py-2 text-sm bg-indigo-600 hover:bg-indigo-700 text-white rounded' onclick='querySlope(${sensor.address})'><i class='fas fa-chart-line'></i> Slope</button>`;}html+=`</div></div>`;}if(type!=='MAX17048'){html+=`<div class='border-t border-gray-300 dark:border-gray-600 pt-4 mt-4'><h4 class='text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3'>Manual Command Mode</h4><div class='bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3'><p class='text-xs text-gray-600 dark:text-gray-400 mb-3'>Send raw I2C commands directly to the sensor for debugging.</p><div class='flex gap-2 mb-2'><input id='manual-cmd-${sensor.address}' type='text' placeholder='e.g. i, R, O,?' class='flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white font-mono text-sm'><button class='px-4 py-2 text-sm bg-indigo-600 hover:bg-indigo-700 text-white rounded font-semibold' onclick='sendManualCommand(${sensor.address})'><i class='fas fa-paper-plane'></i> Send</button></div><div id='manual-response-${sensor.address}' class='text-xs font-mono bg-gray-900 dark:bg-black text-green-400 p-2 rounded min-h-[60px] overflow-auto' style='display:none;'></div></div></div>`;}if(isEc){html+=`<div class='border-t border-gray-300 dark:border-gray-600 pt-4 mt-4'><h4 class='text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3'>EC Advanced Settings</h4><div class='bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg p-3 mb-3'><label class='block text-sm text-gray-700 dark:text-gray-300 mb-2'>Temperature Compensation (°C)</label><div class='flex gap-2'><input id='ec-temp-${sensor.address}' type='number' step='0.1' value='${sensor.ec_temp_comp||25}' class='flex-1 px-2 py-1 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white'><button class='px-3 py-1 text-sm bg-blue-600 text-white rounded' onclick='setEcTempComp(${sensor.address})'>Apply</button></div></div><div class='bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg p-3'><label class='block text-sm text-gray-700 dark:text-gray-300 mb-2'>TDS Conversion Factor</label><input class='w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white' type='number' step='0.01' id='tds-${sensor.address}' value='${sensor.tds_factor??0.5}'><p class='text-xs text-gray-500 dark:text-gray-400 mt-2'>Multiplier for converting EC to TDS (typically 0.5-0.7)</p></div></div>`;}if(type!=='MAX17048'){html+=`<div class='border-t border-gray-300 dark:border-gray-600 pt-4 mt-4'><h4 class='text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3'>Advanced Settings</h4>`;if(supportsTempComp){html+=`<div class='bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg p-3 mb-3'>`;html+=renderPhTempCompSection(sensor);html+=`</div>`;}if(isPh){html+=`<div class='bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg p-3 mb-3'><label class='flex items-center text-gray-700 dark:text-gray-300'><input type='checkbox' class='mr-2' id='extscale-${sensor.address}' ${sensor.extended_scale?'checked':''}> Extended pH Scale (-1 to 15)</label><p class='text-xs text-gray-500 dark:text-gray-400 mt-2'>Extends the readable pH range from 0-14 to -1 to 15</p></div>`;}html+=`<div class='bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg p-3'><label class='flex items-center text-gray-700 dark:text-gray-300'><input type='checkbox' class='mr-2' id='plock-${sensor.address}' ${sensor.plock?'checked':''}> Protocol Lock (I2C Only)</label><p class='text-xs text-gray-500 dark:text-gray-400 mt-2'>Prevents switching between I2C and UART protocols</p></div></div>`;}if(caps.includes('sleep')){html+=`<div class='border-t border-gray-300 dark:border-gray-600 pt-4 mt-4'>`;html+=renderSleepSection(sensor);html+=`</div>`;}html+=`</div>`;return html;}
